...
const ExpandedRow = ({ isDark, log, detailLog, detailLoading, detailError, onClose }) => {
  ...
  return (
    <tr>
      <td colSpan={5} className={clsx('px-6 py-4 border-b', isDark ? 'border-surface-border bg-surface' : 'border-gray-100 bg-gray-50')}>
        ...
        {!detailLoading && !detailError && (
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-x-12 gap-y-0">
            <div className={clsx('divide-y', isDark ? 'divide-surface-border/50' : 'divide-gray-100')}>
              {commonFields.map(f => (
                <FieldRow key={f.key} label={f.label} value={f.value ?? '—'} fieldKey={f.key} full={f.full} dot={f.dot} copyable={f.copyable} />
              ))}
            </div>
            {extendedFields.length > 0 && (
              <div className={clsx('divide-y', isDark ? 'divide-surface-border/50' : 'divide-gray-100')}>
                {extendedFields.map(f => (
                  <FieldRow key={f.key} label={f.label} value={f.value} fieldKey={f.key} copyable={f.copyable} />
                ))}
              </div>
            )}
          </div>
...
const PaginationControls = ({ isDark, currentPage, nextAfter, total, logsCount, onPrev, onNext, onPageInput, cursors }) => {
  ...
  const maxReached  = cursors.length - 1   // highest page walked to so far
        ...
        <div className="flex flex-col items-center gap-0.5">
          <div className="flex items-center gap-1.5">
            ...
            <input
              ...
              title="Type a page number and press Enter — will walk there sequentially if needed"
            />
            {totalPages && (
              <span className={clsx('text-sm font-mono', isDark ? 'text-ink-muted' : 'text-gray-400')}>
          ...
          {/* Hint — shows when jumping forward beyond cached pages */}
...
export default function LogExplorer() {
  ...
  // Passes _index so backend targets one index instead of 7-day pattern
...
// full logs only at destination.
// TODO: if random access becomes critical, switch to from/size pagination
const handlePageInput = useCallback(async (targetPage) => {
  if (targetPage < 0) return

  // Already cached — jump directly
  if (targetPage === 0 || cursors[targetPage] !== undefined) {
    setCurrentPage(targetPage)
    fetchLogs(activeSearch, timeRange, cursors[targetPage] ?? null)
    return
  }

  // Walk forward from last known cursor using size:1 to collect cursors cheaply
  setLoading(true)
  setError(null)

  let localCursors = [...cursors]
  let lastKnown    = localCursors.length - 1          // last page with a cursor
  let lastCursor   = localCursors[lastKnown]

  try {
    while (lastKnown < targetPage) {
      const params = { search: activeSearch, range: timeRange, size: 1 }
    ...
    // Cursors built — now fetch full page at destination
    setCursors(localCursors)
    setCurrentPage(targetPage)
    fetchLogs(activeSearch, timeRange, localCursors[targetPage])

  } catch (err) {
    setError('Failed to jump to page.')
    setLoading(false)
  }
}, [cursors, activeSearch, timeRange, fetchLogs])
  ...
  return (
    <div className={clsx('relative min-h-screen transition-colors duration-300', isDark ? 'bg-surface' : 'bg-gray-100')}>
      ...
      <div className="relative z-10 max-w-screen-2xl mx-auto px-6 xl:px-10 2xl:px-16 py-6 xl:py-8">
        ...
        {/* ── Search bar ── */}
        <div className={clsx('flex items-center gap-3 p-4 rounded-sm mb-4 border', isDark ? 'bg-surface-card border-surface-border' : 'bg-white border-gray-200 shadow-sm')}>
          <div className="relative flex-1">
            ...
            />
            {searchInput && (
              <button onClick={handleClear} className={clsx('absolute right-3 top-1/2 -translate-y-1/2 transition-colors', isDark ? 'text-ink-muted hover:text-ink-secondary' : 'text-gray-400 hover:text-gray-600')}>
        ...
        {logs.length > 0 && !loading && (
          <div className="mb-3"><PaginationControls {...paginationProps} /></div>
        )}
        ...
        <div className={clsx('rounded-sm border overflow-hidden mb-4', isDark ? 'border-surface-border' : 'border-gray-200 shadow-sm')}>
          <table className="w-full text-sm">
            <thead>
              <tr className={clsx('text-sm font-mono tracking-widest uppercase', isDark ? 'bg-surface-card border-b border-surface-border text-ink-secondary' : 'bg-gray-50 border-b border-gray-200 text-gray-500')}>
            ...
            <tbody>
                ...
                <tr key={i} className={clsx('border-b', isDark ? 'border-surface-border' : 'border-gray-100')}>
                  {[1, 2, 3, 4, 5].map((_, j) => (
                    <td key={j} className="px-4 py-3">
                      <div className={clsx('h-3.5 rounded animate-pulse', isDark ? 'bg-surface-border' : 'bg-gray-200', j === 4 ? 'w-3/4' : 'w-full')} />
                    </td>
                  ))}
                </tr>
              ))}
              ...
              {/* ── Log rows ── key on Fragment, fetchDetail gets id + _index ── */}
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className={clsx('shrink-0', isDark ? 'text-ink-muted' : 'text-gray-400')}>
                          {expandedId === log.id ? <ExpandLessIcon sx={{ fontSize: 18 }} /> : <ExpandMoreIcon sx={{ fontSize: 18 }} />}
                        </span>
                      </div>
                    </td>
                  </tr>
